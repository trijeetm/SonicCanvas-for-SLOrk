...
.controller('CanvasCtrl', function($scope, $stateParams, $firebase, $ionicLoading, $ionicHistory) {
  ...
  var root = (($stateParams.canvasId + 5) * 69) % 12;
      ...
      touchmove: function() {
        for ( var i = this.touches.length - 1, touch; i >= 0; i-- ) {
          ...
          if (this.dragging) {
            ...
            patches[brushColor].increaseVolume();
            // patches[brushColor].increaseDensity();

            pixelDataRef.child(
              Date.now() + ':' + Math.floor(touch.ox) + ',' + Math.floor(touch.oy) + ':' + Math.floor(touch.x) + ',' + Math.floor(touch.y)).set(brushColor + ':' + brushRadius
              ); 
          }
        }
      }
...
.controller('CanvasesCtrl', function($scope, $firebase, $ionicModal, $timeout) {
  ...
  $scope.openCanvasModal = function() {
    ...
    $scope.canvasData.scaleRoot = 0;
    $scope.canvasData.scaleType = 0;
  };
...
.controller('ViewerCtrl', function($scope, $firebase, $ionicModal, $timeout) {
  var canvasesRef = new Firebase("https://amber-torch-7567.firebaseio.com/slork/canvases");
  $scope.canvases = $firebase(canvasesRef).$asArray();
...
.controller('ViewCtrl', function($scope, $stateParams, $firebase, $ionicLoading, $ionicHistory) {
  var loading = true;
